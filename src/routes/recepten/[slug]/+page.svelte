    ...
    <div class="mb-4 flex justify-between">
		...
		<h4 class="text-white align-text-bottom">Door:
			...
			{:else if recipeDetailQuery.isError}
				Fout bij het laden van het recept.
			{:else if recipeDetailQuery.data}
				{recipeDetailQuery.data.created_by.title} <img src={recipeDetailQuery.data.created_by.profile_icon} alt="{recipeDetailQuery.data.created_by.title} profiel foto" class="w-6 h-6 rounded-full inline-block ml-2"/>
			{/if}
		</h4>
    </div>
